...
class DIPUStorageImpl : public c10::StorageImpl {
 public:
  explicit DIPUStorageImpl(use_byte_size_t use_byte_size,
      size_t size_bytes,
  ...
  void init_desc(c10::IntArrayRef size, c10::optional<at::MemoryFormat> memory_format_opt);

  void get_desc(diopiStorageDesc_t* desc) const;

  void set_desc(const diopiStorageDesc_t& desc);

  static bool TensorHasStorageDesc(const at::Tensor* tensor);
